 ...
 * @version 1.0
 * Displays a JFrame (frame) containing a random mixture of circles, triangles
 * pentagons and rectangles with random colors, sizes, and positions.
...
public class NervousShapes
{
    ...
    /**
     * Creates a frame labeled "Nervous Shapes", displays the frame, and sets
     * the size of the frame (using the WINDOW_SIZE class variable). Assigns the
     * frame to the df class variable, and assigns the frame's graphics context
     * to the g class variable.
     */
    ...
    /**
     * Creates enough Circle, Pentagon, Triangle and Rectangle objects to fill
     * the shapes array. Each shape has a random color, size, and position. The
     * height and width of each shape must lie between MIN_SIZE and MAX_SIZE
     * (inclusive). The position is chosen so that the shape is completely
    ...
    private static void createShapes()
    {
        for (int i = 0; i < shapes.length; i++)
        {
            ...
            if (randomValue >= 0.60 && randomValue < .80)
            {
    ...
    /**
     * Establishes an infinite loop in which the shapes are animated. During
     * each loop iteration, the drawing area is cleared and the shapes are then
     * drawn at new positions. The new x and y coordinates for each shape will
     * either be the same as the old ones, one pixel smaller, or one pixel
     * larger. A shape is not moved if doing so would cause any portion of the
     * shape to go outside the drawing area. At the end of each animation cycle,
     * there is a brief pause, which is controlled by the delay constant.
     */
     ...
     * @return A random integer that is greater than or equal to min and less
